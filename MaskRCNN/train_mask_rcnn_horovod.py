...
def parse_args():
    ...
    args.lr_decay_epoch = args.lr_decay_epoch if args.lr_decay_epoch else '8,10'
...
def get_dataloader(net, train_dataset, val_dataset, train_transform, val_transform, batch_size,
    ...
    val_loader = mx.gluon.data.DataLoader(
        ...
        batch_size, False, batchify_fn=val_bfn, last_batch='discard', num_workers=args.num_workers)
...
def train(net, train_data, val_data, eval_metric, ctx, args, start_time):
    """Training pipeline"""
    ...
    if args.horovod:
        ...
        trainer = hvd.DistributedTrainer(net.collect_train_params(), opt)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            # logging.info('[Batch Num: {}][Rank {}]'.format(i, rank))
            ...
            # update metrics
            if (not args.horovod or hvd.rank() == 0) and args.log_interval and not (
                                                                                           i + 1) % args.log_interval:
                msg = ','.join(['{}={:.3f}'.format(*metric.get()) for metric in metrics + metrics2])
                logger.info('[Epoch {}][Rank {}][Batch {}], Speed: {:.3f} samples/sec, {}'.format(
                    epoch, hvd.rank(), i,
                    args.log_interval * args.batch_size / (time.time() - btic), msg))
        ...
        if (not args.horovod or hvd.rank() == 0):
            ...
            logger.info('[Epoch {}][Rank {}] Training cost: {:.3f}, {}'.format(
                epoch, hvd.rank(), (time.time() - tic), msg))
            if hvd.rank() == 0:
                epoch_speed = hvd.size() * batch_size * i / elapsed
                logging.info('Epoch[%d]\tSpeed: %.2f samples/sec', epoch, epoch_speed)

            logging.info(
                '[Rank {}]Total Training Duration(Without Saving model): {} secs'.format(hvd.rank(),
        ...
        if not (epoch + 1) % args.val_interval:
            ...
            logging.info('[Rank {}]Total Validation Duration: {} secs'.format(rank, time.time() - start_time_val))
        else:
            current_map = 0.
        '''
        # if (not args.horovod or hvd.rank() == 0):
...
if __name__ == '__main__':
    ...
    train_data, val_data = get_dataloader(
        ...
        batch_size, args.num_workers, args.use_fpn)

    # training
    train(net, train_data, val_data, eval_metric, ctx, args, start_time)

    net.save_parameters('{:s}_best.params'.format(args.save_prefix))

    training_duration_secs = time.time() - start_time
    logging.info('Train + Saved Param Duration: {} secs'.format(training_duration_secs))
    # logging.info(f'Total duration: {0:,.2f}'.format(training_duration_secs))
